function [high,low,ternary]=ltp_image(in,type,threshold)
   ...
   code = 2.^(7:-1:0)';
   high1 = reshape((d>=threshold)*code,rows,cols);
   low1 = reshape((d<=-threshold)*code,rows,cols);
   %high = reshape((d>=threshold)*code,rows,cols);
   %low = reshape((d<=-threshold)*code,rows,cols);
   if nargout>2
      code = 3.^(7:-1:0)';
      ternary1 = reshape((d>=threshold)*code+(d<=-threshold)*2*code,rows,cols);
      %ternary = reshape((d>=threshold)*code+(d<=-threshold)*2*code,rows,cols);
   end 
   
%    high=hist(high1(:),0:255);
%    low=hist(low1(:),0:255);
%    ternary=hist(ternary1(:),0:255);
   ternary = ternary1;
   high = high1;
   low = low1;
%end;