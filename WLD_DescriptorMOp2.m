...
%       T : Number of orientations (4, 6, 8, 10, 12)
%       M : The number of segments to which each sub-histogram is divided
%           (4, 6, 8)
%       S : The number of bins in each segment (5, 10, 15, 20)
%       P : The number points in the neighbourhod (8, 16, 20)
%       R : The radious of the neighbourhood (1, 2, 3)
%
% OUTPUT:
...
%--------------------------------------------------------------------------
function WLDHist = WLD_DescriptorMOp2(Im, T, M, S, P, R)
PIR = 3.141592653589;
PI = 180;
%% Step-1
...
for i =1:M;
    for j = 1:T;
        H_MT = ComputeSegmentHist(HT{j}, ll(i), lu(i), S);
        WLD_H = [WLD_H H_MT];
    end;
end;
clear HT;
